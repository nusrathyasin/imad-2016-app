	...
	request.onreadystatechange=function(){
		if(request.readyState===XMLHttpRequest.DONE)
		{
			if(request.status===200)
			{
				...
				{
				   list="<div class='commentbox'><div style='float:left;width:50%'><b>"+names[i]+"</b></div><div><b>"+names[i+2]+"</b></div><hr/><br/>"+names[i+1]+"</div>" + list;
				i=i+2;
				}
...
request.open('GET','http://nusrathyasin.imad.hasura-app.io/submit-name?name='+name+'&firstname='+firstname,true);
request.send(null);
